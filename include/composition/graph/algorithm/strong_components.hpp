...
std::vector<boost::filtered_graph<graph_t,
                                  ...
                                  std::function<bool(typename graph_t::vertex_descriptor)>>> strong_components(graph_t &g) {
  ...
  size_t num = boost::strong_components(g,
  ...
  for (size_t i = 0; i < num; ++i) {
    component_graphs.push_back({g,
                                ...
                                [mapping, i, index](vd_t vd) {
                                  return mapping->at(index->find(vd)->second) == i;
                                }});
  }
